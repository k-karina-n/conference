@props(['data' => null, 'label' => null, 'type' => null, 'model' => 'firstStep', 'userCountry' => null, 'userPhoto' => null])
...
<div class="space-y-2">
    ...
    @if ($data == 'country')
        <select class="{{ $class }} bg-white" name="country" wire:model="firstStep.country">
            <option>{{ $userCountry ?? 'Select your country' }}</option>
            @foreach ($countries as $country)
                <option>{{ $country }}</option>
            @endforeach
        </select>
    @elseif($data == 'description')
        <textarea class="{{ $class }}" name="description" type="text" wire:model="secondStep.description" rows="3"
            placeholder="Description (up to 1000 characters)" maxlength="1000">
        {{ $slot }}
    </textarea>
    @else
        <input class="{{ $class }}" name="{{ $data }}" type="{{ $type ?? 'text' }}"
            wire:model.lazy="{{ $model }}.{{ $data }}" {{ $attributes->merge() }}>
    @endif
